<template>
    <div class="nice-bar" v-bind:class="[ theme==='dark' ? 'theme-dark' : 'theme-light', classes ? ' ' + classes : '']" ref="scrollContainer">
        ...
        <vertical-scrollbar v-if="ready && vscroll" v-bind:content="{ height: scrollContentHeight }" v-bind:container="{ height: scrollContainerHeight }"
        ...
        <horizontal-scrollbar v-if="ready && hscroll" v-bind:content="{ width: scrollContentWidth }" v-bind:container="{ width: scrollContainerWidth }"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            moveTheScrollbar(options) {
                options = options || {};
                options.top = options.top || 0;
                options.left = options.left || 0;
                if(options.top){
                  this.vMovement = options.top / this.scrollContentHeight * 100  
                } else {
                  this.vMovement = this.top / this.scrollContentHeight * 100  
                }
                if(options.left){
                  this.hMovement = options.left / this.scrollContentWidth * 100
                } else {
        ...
        watch: {
            'top': function(newval){
                var offset = newval,
                    offsetHeight = this.scrollContentHeight - this.scrollContainerHeight;
                this.$emit('scrolled', {offset, offsetHeight})
            }
        }
    }
</script>
